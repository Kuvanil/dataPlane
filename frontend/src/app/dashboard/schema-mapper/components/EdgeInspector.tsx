...
export default function EdgeInspector({
  ...
  if (!edge) {
    return (
      <aside
        ...
        className="w-72 border-l border-border bg-surface-elevated p-4 text-xs text-fg0"
      >
  ...
  return (
    <aside
      aria-label="Edge inspector"
      className="w-72 border-l border-border bg-surface-elevated flex flex-col"
    >
      <div className="p-4 border-b border-border">
        <div className="text-[10px] uppercase tracking-wider text-fg0 font-semibold">
          Target
        </div>
        <div className="mt-1 text-sm font-mono text-indigo-300">
          {edge.target.table}.{edge.target.column}
        </div>
        <div className="mt-1 text-[11px] text-fg0">
          {edge.target.type ?? "?"}{" "}
      ...
      <div className="p-4 border-b border-border">
        ...
        <ul className="mt-1 text-xs text-blue-300 font-mono space-y-0.5">
          {edge.sources.map((s, i) => (
            <li key={i}>
              {s.table}.{s.column}
              <span className="text-fg0 ml-1">({s.type ?? "?"})</span>
            </li>
          ))}
        </ul>
      </div>
      <div className="p-4 border-b border-border">
        <div className="text-[10px] uppercase tracking-wider text-fg0 font-semibold flex items-center justify-between">
      ...
      <div className="p-4 text-[10px] text-fg0 space-y-1">
        <div>
